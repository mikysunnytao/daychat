...
public abstract class MessageContentHolder extends MessageBaseHolder {
    ...
    private final FrameLayout translationContentFrameLayout;
    private final ImageView translationLoadingImage;
    private final LinearLayout translationResultLayout;
    ...
    public void layoutViews(final TUIMessageBean msg, final int position) {
        ...
        if (properties.getAvatarRadius() != 0) {
            ...
            int radius = AdaptScreenUtils.pt2Px(75);//ScreenUtil.dip2px(4);
...
//                    usernameText.setVisibility(properties.getLeftNameVisibility());
        ...
        setMessageAreaPadding(msg.isSelf());
    ...
    protected void setMessageAreaPadding(boolean self) {
        // after setting background, the padding will be reset
        int paddingHorizontal = AdaptScreenUtils.pt2Px(60);//itemView.getResources().getDimensionPixelSize(R.dimen.chat_message_area_padding_left_right);
        int triangleSize = AdaptScreenUtils.pt2Px(15);
        int paddingVertical = AdaptScreenUtils.pt2Px(40);//itemView.getResources().getDimensionPixelSize(R.dimen.chat_message_area_padding_top_bottom);
        if (self) {
            msgArea.setPadding(paddingHorizontal, paddingVertical, paddingHorizontal+triangleSize, paddingVertical);
        }else {
            msgArea.setPadding(paddingHorizontal+triangleSize, paddingVertical, paddingHorizontal, paddingVertical);
