...
public class MixtureTextView extends RelativeLayout
{
    ...
    private final HashSet<Integer> mCorYHashes = new HashSet<Integer>();
    ...
    private final Map<Integer, Point> mViewBounds = new HashMap<Integer, Point>();
        ...
        for (int i = 0; i < destRectSize; i++)
        {
            ...
            if (rs.size() > 1)
            {
                ...
                for (int k = 0; k < lh; k++)
                {
                    ...
                    {
                        inc++;
                        bak.add(index++, Collections.singletonList(new Rect(
                                rs.get(j).left,
