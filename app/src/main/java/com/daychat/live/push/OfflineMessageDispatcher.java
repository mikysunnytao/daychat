...
import com.daychat.live.util.BrandUtil;
import com.daychat.live.util.DemoLog;
...
public class OfflineMessageDispatcher {
    ...
    private static OfflineMessageBean offlineMessageBeanValidCheck(OfflineMessageBean bean) {
        if (bean == null) {
            ...
            String label = String.valueOf(packageManager.getApplicationLabel(MyApp.instance().getApplicationInfo()));
            ToastUtil.toastLongMessage(MyApp.instance().getString(R.string.you_app) + label + MyApp.instance().getString(R.string.low_version));
