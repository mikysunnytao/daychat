...
public class GlideEngine {
    ...
    public static Bitmap loadBitmap(Object imageUrl, int targetImageSize,boolean group) throws InterruptedException, ExecutionException {
        if (imageUrl == null) {
            return null;
        }
        if (imageUrl.equals(0)){
            imageUrl = R.drawable.header_main;
        }
        RequestOptions options ;
        if (group){
            options = new RequestOptions().error(TUIThemeManager.getAttrResId(TUILogin.getAppContext(), R.attr.core_default_user_icon)).circleCrop();
        }else {
            options = new RequestOptions().error(TUIThemeManager.getAttrResId(TUILogin.getAppContext(), R.attr.core_default_user_icon));
        }
    ...
    public static Bitmap loadBitmap(Object imageUrl, int width, int height,boolean group) throws InterruptedException, ExecutionException {
        if (imageUrl == null) {
            return null;
        }
        return Glide.with(TUILogin.getAppContext()).asBitmap()
