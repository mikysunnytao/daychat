...
import com.tencent.qcloud.tuicore.interfaces.LoadCompleteListener;
...
//        groupAvatar.setRadius(radius);
    ...
    public void onClick(View v) {
        if (v == groupAvatar || v == groupAvatarLayout) {
            ...
            popupInputCard.setContent(groupNameLv.getContent());
            ...
            PopupInputCard popupInputCard = new PopupInputCard(this);
            popupInputCard.setContent(groupIdLv.getContent());
    ...
    private List<Object> fillGroupGridAvatar() {
        ...
        if (TextUtils.isEmpty(savedIcon)) {
            int faceSize = Math.min(mGroupMembers.size(), 4);
    ...
    private void createGroupChat() {
        ...
        Bitmap[] headers;
        if (mGroupMembers.size()>=4){
            headers = new Bitmap[4];
        }else {
            headers = new Bitmap[mGroupMembers.size()];
        }
//        final int[] j = {0};
//        for (int i = 0; i < mGroupMembers.size(); i++) {
//            GroupMemberInfo mGroupMember = mGroupMembers.get(i);
//            ImageUtil.returnBitMap(mGroupMember.getIconUrl(), new LoadCompleteListener() {
//                @Override
//                public void loaded(Bitmap bitmap) {
//                    Bitmap roundBitmap = ImageUtil.toRoundBitmap(bitmap);
//                    headers[j[0]] = roundBitmap;
//                    j[0]++;
//                }
//            });
//            if (j[0] >=4){
...
//        CombineBitmap.init(this).setBitmaps(headers);

//        ImageUtil.storeBitmap()
//        groupInfo.setFaceUrl();
